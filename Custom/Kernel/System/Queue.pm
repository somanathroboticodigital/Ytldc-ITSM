...
sub QueueAdd {
    ...
    if ( !$Param{NoDefaultValues} ) {
        for (
            ...
            FollowUpID FollowUpLock DefaultSignKey Calendar TeamManager)
            )
        {
    ...
    return if !$DBObject->Do(
        SQL => 'INSERT INTO queue (name, group_id, manager_id, unlock_timeout, system_address_id, '
        ...
        Bind => [
            \$Param{Name},              \$Param{GroupID},             \$Param{TeamManager},       \$Param{UnlockTimeout}, \$Param{SystemAddressID},
...
sub QueueGet {
    ...
    my $SQL = 'SELECT q.name, q.group_id, q.unlock_timeout, '
        ...
        . 'q.default_sign_key, q.calendar_name, q.create_time, q.change_time,q.manager_id FROM queue q, '
...
sub QueueUpdate {
    ...
    # check needed stuff
    for (
        qw(QueueID Name ValidID GroupID TeamManager SystemAddressID SalutationID SignatureID UserID FollowUpID)
        )
    {
        if ( !$Param{$_} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
    ...
    return if !$DBObject->Do(
        SQL => '
            UPDATE queue
            SET name = ?, comments = ?, group_id = ?, manager_id = ?, unlock_timeout = ?, first_response_time = ?,
            ...
            WHERE id = ?',
        Bind => [
            \$Param{Name},              \$Param{Comment},             \$Param{GroupID}, \$Param{TeamManager}, \$Param{UnlockTimeout},
            \$Param{FirstResponseTime}, \$Param{FirstResponseNotify}, \$Param{UpdateTime},
            \$Param{UpdateNotify},      \$Param{SolutionTime},        \$Param{SolutionNotify},
