...
sub CustomerCompanyAdd {
    ...
    my @keys_to_compare = qw(CustomerID CustomerCompanyName CustomerCompanyStreet CustomerCompanyCity CustomerCompanyCountry CustomerCompanyComment ValidID);
    ...
    foreach my $key (@keys_to_compare) {
        ...
        if (exists $Param{$key} && $Param{$key} ne '') {
            ...
            my $AgentHistoryAdd = $Self->CustomerHistoryAdd(
                Name            => $history_name,
...
sub CustomerCompanyUpdate {
    ...
    foreach my $key (@keys_to_compare) {
        # If the key exists in both hashes and the values are different
        if (exists $Company{$key} && exists $Param{$key} && $Company{$key} ne $Param{$key}) {

            my $history_name = "Changed $key from \"$Company{$key}\" to \"$Param{$key}\".";
...
sub CustomerHistoryAdd {
    ...
    # check needed stuff
    for my $Needed (qw(CustomerID OwnerID CreateUserID )) {
        if ( !$Param{$Needed} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Needed!"
    ...
    # db insert
        return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => 'INSERT INTO customer_history '
                . ' (name, customer_id, owner_id, '
                . ' create_time, create_by, change_time, change_by) '
                . 'VALUES '
                . '(?, ?, ?, current_timestamp, ?, current_timestamp, ?)',
            Bind => [
                \$Param{Name}, \$Param{CustomerID}, \$Param{OwnerID}, \$Param{CreateUserID},  \$Param{CreateUserID},
            ],
        );
