...
sub UserAdd {
    ...
    my @keys_to_compare = qw(UserTitle UserFirstname UserLastname UserLogin UserEmail UserMobile  ValidID);
    ...
    foreach my $key (@keys_to_compare) {
        ...
        if (exists $Param{$key} && $Param{$key} ne '') {
            ...
            my $AgentHistoryAdd =  $Self->AgentHistoryAdd(
                Name            => $history_name,
                AgentID         => $UserID,
                OwnerID         => $Param{ChangeUserID},
                CreateUserID    => $Param{ChangeUserID},
            );
        }
    }
...
sub UserUpdate {
    ...
    foreach my $key (@keys_to_compare) {
        # If the key exists in both hashes and the values are different
        if (exists $UserData{$key} && exists $Param{$key} && $UserData{$key} ne $Param{$key}) {

            my $history_name = "Changed $key from \"$UserData{$key}\" to \"$Param{$key}\".";

            my $AgentHistoryUpdate = $Self->AgentHistoryAdd(
                Name            => $history_name,
                AgentID         => $Param{UserID},
                OwnerID         => $Param{ChangeUserID},
                CreateUserID    => $Param{ChangeUserID},
            );
        }
...
sub AgentHistoryAdd {
    ...
    # check needed stuff
    for my $Needed (qw(AgentID OwnerID CreateUserID )) {
        if ( !$Param{$Needed} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Needed!"
    ...
    # db insert
        return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => 'INSERT INTO agent_history '
                . ' (name, agent_id, owner_id, '
                . ' create_time, create_by, change_time, change_by) '
                . 'VALUES '
                . '(?, ?, ?, current_timestamp, ?, current_timestamp, ?)',
            Bind => [
                \$Param{Name}, \$Param{AgentID}, \$Param{OwnerID}, \$Param{CreateUserID},  \$Param{CreateUserID},
            ],
        );
