...
sub CustomerUserAdd {
    ...
    $Kernel::OM->Get('Kernel::System::KeycloakAuth')->AddUserInKeycloak(
        %Param,
        UserType => 'CustomerUser',
        ChangeUserID => $Param{UserID},
        UserPw => $Param{UserPassword} 
    );

    my @keys_to_compare = qw(UserFax UserEmail UserLogin UserComment UserCountry UserStreet UserLastname UserCity UserTitle UserCustomerID  UserZip UserMobile UserPhone ValidID);
    my $AgentHistoryAdd;
    foreach my $key (@keys_to_compare) {
            # If the key exists in both hashes and the values are different
        if (exists $Param{$key} && $Param{$key} ne '') {

            my $history_name = "Created $key with $Param{$key}.";

            $AgentHistoryAdd = $Self->CustomerUserHistoryAdd(
                Name            => $history_name,
                CustomerUserID  => $Param{UserLogin},
...
sub CustomerUserUpdate {
    ...
    $Kernel::OM->Get('Kernel::System::KeycloakAuth')->UpdateUserInKeycloak(
        %Param,
        UserType => 'CustomerUser',
        OldUserLogin => $User{UserLogin},
        ChangeUserID => $Param{UserID},
    );
    ...
    my @keys_to_compare = qw(UserFax UserEmail UserLogin UserComment UserCountry UserStreet UserLastname UserCity UserTitle UserCustomerID  UserZip UserMobile UserPhone ValidID);
            ...
            foreach my $key (@keys_to_compare) {
                # If the key exists in both hashes and the values are different
                if (exists $User{$key} && exists $Param{$key} && $User{$key} ne $Param{$key}) {

                    my $history_name = "Changed $key from \"$User{$key}\" to \"$Param{$key}\".";

                    my $AgentHistoryAdd = $Self->CustomerUserHistoryAdd(
                        Name            => $history_name,
...
sub CustomerUserHistoryAdd {
    ...
    # check needed stuff
    for my $Needed (qw(CustomerUserID OwnerID CreateUserID )) {
        if ( !$Param{$Needed} ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Need $Needed!"
    ...
    # db insert
        return if !$Kernel::OM->Get('Kernel::System::DB')->Do(
            SQL => 'INSERT INTO customer_user_history '
                . ' (name, login_id, owner_id, '
                . ' create_time, create_by, change_time, change_by) '
                . 'VALUES '
                . '(?, ?, ?, current_timestamp, ?, current_timestamp, ?)',
            Bind => [
                \$Param{Name}, \$Param{CustomerUserID}, \$Param{OwnerID}, \$Param{CreateUserID},  \$Param{CreateUserID},
            ],
        );
