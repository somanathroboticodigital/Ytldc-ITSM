...
package Kernel::System::Console::Command::Maint::Ticket::FirstResponseTimeEscalationCheck;
...
sub Run {
    ...
    for my $TicketID (@Tickets) {
        ...
        TYPE:
        for my $Type (
            qw(FirstResponseTimeEscalation)
            )
        {
            ...
            my $HistoryLinesCount  = grep { $_->{HistoryType} eq $TicketAttr2Event{$Type} } @ReversedHistoryLines;

            
            if ($DecayTimeInSeconds and $HistoryLinesCount == 1) {
                ...
                if ( $PrevEventLine && $PrevEventLine->{CreateTime} ) {
                    ...
                    if($TimeSincePrevEvent <= 660){
                        $TicketObject->EventHandler(
                            Event  => 'NotificationEscalationResponseAfterTenMin',
                            Data   => {
