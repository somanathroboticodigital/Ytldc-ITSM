...
sub Content {    ## no critic qw(Subroutines::RequireFinalReturn)
    ...
    my $KeycloakUserDetails;
    if($code){

        my $KeycloakAuthObject = $Kernel::OM->Get('Kernel::System::KeycloakAuth');
        $KeycloakUserDetails = $KeycloakAuthObject->GetauthtokenAgent( 
            code => $code,
        );

        if(defined $KeycloakUserDetails->{preferred_username} and defined $KeycloakUserDetails->{email_verified} and $KeycloakUserDetails->{preferred_username} and $KeycloakUserDetails->{email_verified}){
            $Param{Action} = 'Login';
            $Param{code} = $code;
        }
    ...
    if ( $Param{Action} eq 'PreLogin' ) {
        ...
        if(defined $KeycloakUserDetails->{preferred_username} and defined $KeycloakUserDetails->{email_verified} and $KeycloakUserDetails->{preferred_username} and $KeycloakUserDetails->{email_verified}){
            $User = $KeycloakUserDetails->{preferred_username} ;
        }else{
    ...
    # logout
        ...
        my $KeycloakAuth = $Kernel::OM->Get('Kernel::System::KeycloakAuth');
        my %KeycloakData = $KeycloakAuth->GetAccessTokenDetails(
            UserID => $UserData{UserID},
            UserType => 'Agent'
        );

        $KeycloakAuth->DeleteUserAccessToken(
            UserID => $UserData{UserID},
            UserType => 'Agent'
        );
        ...
        if ( $ConfigObject->Get('LogoutURL') ) {
            my $logout_url = $ConfigObject->Get('LogoutURL') . $KeycloakData{IDToken};
            $LayoutObject->Redirect(
                ExtURL =>  $logout_url,
            );    # throws a Kernel::System::Web::Exception
        }
    ...
    # show login site
        ...
        if ( $AuthObject->GetOption( What => 'PreAuth' ) ) {
            ...
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  =>" Login Page details <> $Param{RequestedURL}" ,
            );
