...
sub Run {
    ...
    # create new ticket and article
        ...
        if ( !$ExpandCustomerName ) {
            ...
            if ( $ConfigObject->Get('Ticket::Frontend::AgentTicketPhone::TicketServiceSLA') ) {
        ...
        if ($DynamicFieldValues{Severity} eq '1-High' || $DynamicFieldValues{Severity} eq '2-Medium') {
            $NewQueueID = 6;
        }
        if ($DynamicFieldValues{Severity} eq '3-Low' || $DynamicFieldValues{Severity} eq '4-Very Low') {
            $NewQueueID = 7;
        }
        ...
        if ($TicketID) {
            ...
            my @TempLinkAssetData = $Kernel::OM->Get('Kernel::System::AssetLink')->TempLinkAssetGet(FormID => $Self->{FormID});
            foreach my $AssetID (@TempLinkAssetData) {
                ...
                $TicketObject->HistoryAdd(
                    ...
                    CreateUserID => 1,
                    HistoryType  => 'AddNote',
                    Name         => "\%\%Added Asset link with AssetID $AssetID",
                );
            }
        }
