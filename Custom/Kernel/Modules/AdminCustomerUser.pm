...
sub _Edit {
    ...
    if ( $Param{Action} eq 'Change' ) {
        ...
        $Self->_CustomerUserDetails(%Param);
    }
...
sub _CustomerUserDetails {
    ...
    my @CustomerUserDeatils = $Self->CustomerUserHistoryGet(
        LoginID         => $Param{ID }
    );


    my $Time;
    for my $Data (@CustomerUserDeatils) {
...
sub CustomerUserHistoryGet {
    ...
    return if !$DBObject->Prepare(
        SQL => 'SELECT name,login_id ,owner_id,create_time,create_by,change_time,change_by from customer_user_history where login_id = ? ',
        Bind => [ \$Param{LoginID} ],
    );
