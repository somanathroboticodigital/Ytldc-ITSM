...
package Kernel::Modules::LinkAssetTemporary;
...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'AssetData' ) {
        ...
        # my $AssetStatus = $ParamObject->GetParam( Param => 'assetstatus' );

        if ($AssetUUID or $AssetName) {
            ...
            $LayoutObject->Block(
                Name => 'LinkTable',
                Data => {AssetID => $AssetUUID, FormID => $FormID}
            );
    ...
    if ($Self->{Subaction} eq 'AssetLink') {
        ...
        my $FormIDSelAsset = $ParamObject->GetParam( Param => 'FormID' );
        ...
        foreach my $ID (@AssetID) {
        	if ($ID ne '') {
	            ...
	            my $TempAssetlinkAdd    => $AssetObject->TempAssetlinkAdd(
	                FormID              => $FormIDSelAsset,
	                AssetID             => $ID,
	            );
     ...
     my $TempAssetData = $AssetObject->AssetTempGet( FormID => $FormID);

    if ($TempAssetData) {
         # body...  
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Values >>> $TempAssetData",
            );
            $LayoutObject->Block(
                Name => 'Delete',
                Data => {
                    AssetID => $TempAssetData,
                },
            );
     } 
    ...
    $Output .= $LayoutObject->Output(
        TemplateFile => 'LinkAssetTemporary',
        Data         => { FormID => $FormID,ManageTabDisabled  => $ManageTabDisabled,ActiveTab => $ActiveTab, }
    );
