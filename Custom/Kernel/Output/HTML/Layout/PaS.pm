...
package Kernel::Output::HTML::Layout::PaS;
...
sub PaSListShow {
    ...
    if ( !$Param{View} && $Self->{ 'UserPaSOverview' . $Env->{Action} } ) {
        $Param{View} = $Self->{ 'UserPaSOverview' . $Env->{Action} };
    }
    ...
    $Kernel::OM->Get('Kernel::System::AuthSession')->UpdateSessionID(
        ...
        Key       => 'UserPaSOverview' . $Env->{Action},
        Value     => $View,
    );
     open(TT, '>', '/tmp/file2.log');
        use Data::Dumper;
    ...
    my $Backends = $ConfigObject->Get('PaS::Frontend::Overview');
    if ( !$Backends ) {
        return $Self->FatalError(
            Message => 'Need config option PaS::Frontend::Overview',
        );
    }
    ...
    if ( ref $Backends ne 'HASH' ) {
        return $Self->FatalError(
            Message => 'Config option PaS::Frontend::Overview needs to be a HASH ref!',
        );
    }
    ...
    my $PageShownPreferencesKey = 'UserPaSOverview' . $View . 'PageShown';
    my $PageShown               = $Self->{$PageShownPreferencesKey} || 10;
    my $Group                   = 'PaSOverview' . $View . 'PageShown';
    ...
    my $OutputNavBar = $Self->Output(
        TemplateFile => 'AgentPaSOverviewNavBar',
        Data         => {%Param},
    );
