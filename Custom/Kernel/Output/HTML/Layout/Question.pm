...
package Kernel::Output::HTML::Layout::Question;
...
=item ProductsListShow()

Returns a list of Productss list with pagination.

This function is similar to L<Kernel::Output::HTML::Layout::Ticket::TicketListShow()>
in F<Kernel/Output/HTML/Layout/Ticket.pm>.

    my $Output = $LayoutObject->ProductsListShow(
        ProductsIDs  => $ProductsIDsRef,                      # total list of Productss ids, that can be listed
        Total      => scalar @{ $ProductsIDsRef },          # total number of list items, changes in this case
        View       => $Self->{View},                      # optional, the default value is 'Small'
        Filter     => 'All',
        Filters    => \%NavBarFilter,
        FilterLink => $LinkFilter,
        TitleName  => 'Overview: Products',
...
sub QuestionListShow {
    ...
    if ( !$Param{View} && $Self->{ 'UserQuestionOverview' . $Env->{Action} } ) {
        $Param{View} = $Self->{ 'UserQuestionOverview' . $Env->{Action} };
    }
    ...
    $Kernel::OM->Get('Kernel::System::AuthSession')->UpdateSessionID(
        ...
        Key       => 'UserQuestionOverview' . $Env->{Action},
        Value     => $View,
    );
    ...
    my $Backends = $ConfigObject->Get('Question::Frontend::Overview');
    if ( !$Backends ) {
        return $Self->FatalError(
            Message => 'Need config option Question::Frontend::Overview',
        );
    }
    ...
    if ( ref $Backends ne 'HASH' ) {
        return $Self->FatalError(
            Message => 'Config option Question::Frontend::Overview needs to be a HASH ref!',
        );
    }
    ...
    my $PageShownPreferencesKey = 'UserProductsOverview' . $View . 'PageShown';
    my $PageShown               = $Self->{$PageShownPreferencesKey} || 10;
    my $Group                   = 'QuestionOverview' . $View . 'PageShown';
    ...
    # build shown Productss per page
    ...
    my $OutputNavBar = $Self->Output(
        TemplateFile => 'AgentQuestionOverviewNavBar',
        Data         => {%Param},
    );
